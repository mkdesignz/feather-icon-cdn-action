...
for (const icon in icons) {
  iconList.push({
    name: icon,
    svg: `data:image/svg+xml;base64,${Buffer.from(icons[icon].toSvg()).toString('base64')}`,
    color: '#000000',
    size: 24,
    url: `${process.env.CYCLIC_URL ? process.env.CYCLIC_URL : process.env.APP_URL ? process.env.APP_URL : ''}/api/icons/${icon}`
  });
}
